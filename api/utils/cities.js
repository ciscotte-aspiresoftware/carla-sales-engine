...
// ─── Supabase layer (geocoded_cities) ───────────────────────────────────────
// findCityAsync is already async, so the geocode cache can read/write Supabase
// directly (no boot-cache needed).
const { isEnabled, getClient } = require('../db');

function geocodeRowToObj(r) {
    return {
        key: r.key, label: r.label, country: r.country, domain: r.domain, language: r.language,
        lat: r.lat, lng: r.lng, ll: r.ll, metro_radius_km: r.metro_radius_km,
        geocoded: r.geocoded, geocodeSource: r.geocode_source, photonProps: r.props,
    };
}
function geocodeObjToRow(key, o) {
    return {
        key, label: o.label || null, country: o.country || null, domain: o.domain || null,
        language: o.language || null, lat: o.lat ?? null, lng: o.lng ?? null, ll: o.ll || null,
        metro_radius_km: o.metro_radius_km ?? null, geocoded: o.geocoded !== false,
        geocode_source: o.geocodeSource || null, props: o.photonProps || null,
        created_at: new Date().toISOString(),
    };
}

async function getGeocodeCached(key) {
    if (isEnabled()) {
        try {
            const { data } = await getClient().from('geocoded_cities').select('*').eq('key', key).maybeSingle();
            return data ? geocodeRowToObj(data) : null;
        } catch { return null; }
    }
    return readGeocodeCache()[key] || null;
}

async function putGeocodeCached(key, obj) {
    if (isEnabled()) {
        try {
            const { error } = await getClient().from('geocoded_cities').upsert(geocodeObjToRow(key, obj), { onConflict: 'key' });
            if (error) console.warn('[Cities] supabase geocode write failed:', error.message);
        } catch (e) { console.warn('[Cities] supabase geocode write threw:', e.message); }
        return;
    }
...
// Map Photon's `properties.countrycode` (lowercased ISO 3166-1 alpha-2) to
// our internal country code + Google domain + default search language. Must
// cover every country in the ICP picker (UK/US/NL/IE/BE/CA/FR/DE/ES/IT/AU/PT);
// otherwise the geocoder falls back to 'US' and a Berlin lookup ends up
// labeled as American Berlin (root cause of the "Berlin → US" bug).
const ISO_TO_INTERNAL = {
    gb: 'UK', us: 'US', ca: 'CA', nl: 'NL', ie: 'IE', be: 'BE',
    de: 'DE', fr: 'FR', es: 'ES', it: 'IT', au: 'AU', pt: 'PT',
};
...
// when we get there. Australia is English even though we route to google.com.au.
const INTERNAL_TO_LANGUAGE = {
    US: 'en', UK: 'en', CA: 'en', NL: 'nl', IE: 'en', BE: 'nl',
    DE: 'de', FR: 'fr', ES: 'es', IT: 'it', AU: 'en', PT: 'pt',
};
...
async function geocodeCity(name) {
    ...
    // Known internal code wins. For unknown countries, surface the raw
    // ISO uppercased rather than silently defaulting to 'US' (which was
    // the root cause of the Berlin → US bug: any country code outside the
    // 6-entry map got stamped American). A city in Vienna will end up
    // country='AT' with google.com / 'en' fallback downstream - the chip
    // is honest, even if domain/language aren't tuned for that market.
    const internal = ISO_TO_INTERNAL[iso] || (iso ? iso.toUpperCase() : 'US');
...
async function findCityAsync(name) {
    ...
    const cached = await getGeocodeCached(cacheKey);
    if (cached) {
        // Self-heal: if Photon's raw countrycode (stored in photonProps) maps
        // to a different internal code than what's cached, the cached row
        // pre-dates the ISO_TO_INTERNAL extension and is stale. Refetch on
        // next read. Catches the historical Berlin → US poisoning without
        // needing a manual DB cleanup. Cache rows without photonProps (very
        // old) get left alone - no information to validate against.
        const cachedCc = String(cached?.photonProps?.countrycode || '').toLowerCase();
        if (cachedCc) {
            const expected = ISO_TO_INTERNAL[cachedCc] || cachedCc.toUpperCase();
            if (expected === cached.country) return cached;
            console.log(`[Cities] cache stale for "${name}": cached country=${cached.country}, photon says ${expected} - refetching`);
            // Fall through to the geocode path below; putGeocodeCached will
            // upsert (overwrite) the bad row.
        } else {
            return cached;
        }
    }
    try {
        ...
        await putGeocodeCached(cacheKey, fresh);
