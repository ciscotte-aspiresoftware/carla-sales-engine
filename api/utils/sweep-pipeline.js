...
const { listIcps, getIcp, pickTermsForCell, computeIcpDefinitionHash } = require('./icps');
// Lazy-required inside sweepCell to break the circular dependency with
// grid-cron (which requires { sweepCell } from this file). A top-level
// require sees grid-cron's exports before they are populated, so the
// destructured isPauseRequested resolves to undefined - and grid-cron
// in turn sees an empty exports object from us, so its `sweepCell` is
// undefined → "sweepCell is not a function" at first tick.
function isPauseRequested() {
    return require('./grid-cron').isPauseRequested();
}
const { getAi, getCellGeneration } = require('./settings');
const { extractContacts, hasAnyContact } = require('./contact-extractor');
...
// Extra schema we tack on top of the ICP-authored classifyPrompt so the
// classifier ALSO surfaces verbatim quotes and key signals as part of the
// SAME GPT call - no extra request, ~200 extra output tokens. Lets the
// company-detail UI show "the website literally says: '…'" instead of
// only the model's free-text reason. If GPT omits the fields we just
// store empty arrays - the UI degrades silently.
...
async function classify(markdown, pageTitle, classifyPrompt) {
    ...
    try {
        ...
        // Defensive: GPT sometimes returns longer quotes than asked. Trim each
        // to 120 chars and cap the list to 3 / signals to 5.
        ...
        return { is_match: false, reason: `classify error: ${err.message}`, key_quotes: [], signals: [] };
    }
}
...
async function fanoutClassify(domain, vertical, primaryIcpId, parentCity) {
    ...
    for (const meta of siblingMetas) {
        ...
        try {
            ...
            // Stamp the sibling's definition hash so the Reclassify tab can
            // tell this fanout-written verdict from one made under a later
            // edited prompt. See utils/icps.js#computeIcpDefinitionHash.
            verdict.definitionHash = computeIcpDefinitionHash(sibling);
...
async function sweepCell(icp, cell) {
    ...
    // Live pipeline: Scrapingdog → chain/type filter → domain dedup against
    // companies.json → per-survivor Firecrawl scrape (cached) → GPT classify
    // → upsertCompany → auto-fanout to sibling ICPs that share this vertical.
    ...
    // Resume-from-checkpoint state. Populated by the checkpoint detection
    // block below; the per-company loop reads `resumeFromIdx` and `fresh`
    // is either re-built from Scrapingdog (fresh sweep) or hydrated from
    // the checkpoint's survivors snapshot (resume).
    let resumeFromIdx = 0;
    let fresh = null;
    // Pass the original survivors list (pre-dedup) along for the checkpoint
    // we may write on pause - lets the resume avoid Scrapingdog AND the
    // dedup-against-known-domains pass.
    let allTerms = null;

    try {
        // ── Resume path: checkpoint present → skip Scrapingdog/filter/dedup ──
        // sweepCell can be called on a `pending` cell that still carries a
        // `pause_checkpoint` from a prior aborted sweep. When present, we
        // restore the in-flight state and jump straight to the per-company
        // loop at the next un-processed company. Scrapingdog credits, term
        // dedup, and dedup-against-known-domains are all already accounted
        // for in the checkpoint's `cumulative` block.
        if (cell.pauseCheckpoint && cell.pauseCheckpoint.stage === 'companies') {
            const cp = cell.pauseCheckpoint;
            const cum = cp.cumulative || {};
            placesFound = cum.placesFound || 0;
            leadsQualified = cum.leadsQualified || 0;
            chainsFiltered = cum.chainsFiltered || 0;
            nonTargetFiltered = cum.nonTargetFiltered || 0;
            alreadyKnown = cum.alreadyKnown || 0;
            if (Array.isArray(cum.touchedDomains)) {
                for (const d of cum.touchedDomains) touchedDomains.push(d);
            }
            fresh = Array.isArray(cp.survivors) ? cp.survivors : [];
            resumeFromIdx = Math.max(0, Math.min(fresh.length, cp.nextIdx || 0));
            allTerms = Array.isArray(cp.allTerms) ? cp.allTerms : [];
            console.log(`[Sweep] ↻ RESUMING ${cell.parentCity}/${cell.id.slice(0, 8)} from checkpoint @ company ${resumeFromIdx + 1}/${fresh.length} (placesFound=${placesFound}, leadsQualified=${leadsQualified})`);
            pushEvent({
                type: 'cell_start',
                icpId: icp.id,
                cellId: cell.id,
                parentCity: cell.parentCity || null,
                message: `Resuming ${cell.parentCity || 'cell'} from company ${resumeFromIdx + 1}/${fresh.length}`,
            });
        }

        // Step 1: Scrapingdog Search (5 credits per term per cell).
        //
        // SKIP when resuming from a checkpoint - the survivors list is already
        // in scope (hydrated above from cell.pauseCheckpoint), and the term
        // dedup + filter passes have already been credited. Going through
        // Scrapingdog again would re-Maps (refunded by search_log dedup) AND
        // re-filter AND re-dedup, all of which are no-ops thanks to caches,
        // but would also overwrite our hydrated `placesFound`/`fresh` with
        // recomputed values that could differ if the ICP changed since pause.
        if (fresh === null) {
        //
        ...
        // Pick the search-term list using the central precedence rules
        // (cityTerms > searchTermsByCountry > flat > skip). See
        // utils/icps.js#pickTermsForCell for the full rationale. The skip
        // case means: outlier cell (its country isn't in icp.countries) with
        // no city-level override - the UI grays it, so we honour that here
        // by NOT calling Scrapingdog (which would waste credits on
        // wrong-language terms).
        const pick = pickTermsForCell(icp, cell);
        const cellCountry = String(cell.country || '').toUpperCase();
        if (pick.skip) {
            console.log(`[Sweep] ⊘ ${cell.parentCity || 'cell'}/${cell.id.slice(0, 8)} - country=${cellCountry} not in ICP countries (${(icp.countries || []).join(',')}) and no city override - skipping`);
            pushEvent({
                type: 'cell_complete',
                icpId: icp.id,
                cellId: cell.id,
                parentCity: cell.parentCity || null,
                message: `Skipped ${cell.parentCity || 'cell'} - ${cellCountry || 'unknown country'} not ticked on this ICP`,
            });
        ...
        // Admin-tunable: how many Scrapingdog pages to fetch per term.
        // Each page = 20 results = 5 credits. Pages offset by 20 per the
        // Scrapingdog docs (page=0 → 1-20, page=20 → 21-40, ...).
        const maxPages = Math.max(1, Math.min(parseInt(getCellGeneration().maxPagesPerSearch, 10) || 1, 6));
        ...
        outer: for (const term of newTerms) {
            ...
            for (const pageOffset of pageOffsets) {
                ...
                const got = (termResults || []).length;
                termCount += got;
                allRaw.push(...(termResults || []));
                // Stop paginating early when a page returns fewer than 20
                // results - Google/Scrapingdog has no more for this query at
        ...
        pushEvent({
            ...
            message: `Found ${allRaw.length} raw places across ${newTerms.length} term${newTerms.length === 1 ? '' : 's'}${maxPages > 1 ? ` (up to ${maxPages} pages each)` : ''}`,
        });
        ...
        //
        // TEMPORARY (demo): the Maps category allowlist (isTargetType, see
        // chains.js) only covers the original car-rental + garden-centre
        // verticals, so brand-new verticals (e.g. the SaaS-reseller ICP) get
        // all their candidates dropped before GPT. We bypass the category
        ...
        for (const r of rawResults) {
            ...
            if (enableTypeFilter && !isTargetType(types)) { nonTargetFiltered++; continue; }
        ...
        } // ← end of `if (fresh === null)` fresh-sweep guard
        ...
        // When the operator clicks Pause mid-cell, the in-flight iterations
        // have already had their scrape kicked off via prevScrape's chain.
        // We let each iteration's CLASSIFY+UPSERT IIFE check `isPauseRequested()`
        // right before it does work; the first one that bails records its
        // 0-based index here. After `await prevClassify` drains the tail,
        // we write a checkpoint with nextIdx = firstSkippedIdx and return.
        let firstSkippedIdx = -1;

        for (let placeIdx = resumeFromIdx; placeIdx < fresh.length; placeIdx++) {
            const place = fresh[placeIdx];
            // 1-based for display + activity events; matches what the user
            // sees in the progress bar.
            const companyIdx = placeIdx + 1;
            ...
            if (!place.website) {
                ...
                prevClassify = (async () => {
                    ...
                    // Mid-cell pause check (no-website branch). Identical to
                    // the main classify IIFE: if the operator requested a pause
                    // while we were waiting on the previous iteration's tail,
                    // skip this iteration's work and record where we stopped.
                    if (isPauseRequested()) {
                    ...
                    try {
                        await upsertCompany({
                            ...
                            // Keep the Google Maps facts even though we can't
                            // scrape/classify - they're all a human needs to
                            // look the business up (the Accounts "needs check"
                            // lane surfaces these). is_match stays null so it
                            // doesn't pollute qualified/rejected counts.
                            classification: {
                                is_match: null,
            ...
            prevClassify = (async () => {
                ...
                if (!scrapeResult || !scrapeResult.ok) {
                    ...
                    try {
                        await upsertCompany({
                            ...
                            vertical: icp.vertical || null,
                            city: cell.parentCity || null,
                            // Preserve the Google Maps facts so the record is
                            // still useful (Database display + Accounts "needs
                            // check" lane) even though the scrape failed.
                            classification: {
                                is_match: null,
                                reason: `scrape error: ${err.message?.slice(0, 200)}`,
                                title: place.title,
                                phone: place.phone,
                                address: place.address,
                                rating: place.rating,
                ...
                try {
                    ...
                    // Harvest contacts straight from the scraped markdown -
                    // emails/phones/LinkedIn the site lists publicly. Free
                    // (no extra API call) and often the only reachable
                    // contact for a micro-business Apollo doesn't index.
                    const contacts = extractContacts(markdown);
                    // Optional per-ICP markdown report (matched → full template
                    // report, rejected → short why-rejected). Second GPT call
                    // on the reportModel, only when the ICP opts in.
                    let report = null;
                    ...
                    const saved = await upsertCompany({
                        ...
                        classification: {
                            ...
                            rating: place.rating,
                            reviews: place.reviews,
                            // Verbatim excerpts + structured signals piggybacked on the same
                            // classify call (no extra GPT cost). Powers "the website literally
                            // says: …" rendering on the company detail.
        ...
        // Paused mid-cell: write a checkpoint and bail BEFORE the normal
        // completion path. The cell stays in `state='pending'` with the
        // checkpoint set, so the next time the cron picks it up sweepCell
        // resumes from `nextIdx` instead of restarting Scrapingdog. The
        // already-processed companies (idx 0..firstSkippedIdx-1) are already
        // upserted to the companies store with their classifications; resume
        // doesn't re-process them.
        if (firstSkippedIdx >= 0) {
            const checkpoint = {
                stage: 'companies',
                survivors: fresh,
                nextIdx: firstSkippedIdx,
                allTerms: allTerms || [],
                cumulative: {
                    placesFound,
                    leadsQualified,
            ...
            console.log(`[Sweep] ⏸ ${cell.parentCity}/${cell.id.slice(0, 8)} PAUSED at company ${firstSkippedIdx + 1}/${fresh.length} (processed=${firstSkippedIdx}, qualified-so-far=${leadsQualified})`);
            pushEvent({
                type: 'cell_complete',
                icpId: icp.id,
                cellId: cell.id,
                ...
                message: `${cell.parentCity || 'cell'} paused at company ${firstSkippedIdx + 1}/${fresh.length} - resume to finish`,
            });
            return { state: 'pending', placesFound, leadsQualified, paused: true };
        }

        // Three-way completion classification (renders distinctly on the globe):
        //   complete (green)  - at least one NEW survivor classified as a match
        //   no_new   (yellow) - we found places but they were all already
        //                       known to us from a prior sweep. The area
        //                       isn't empty - we just don't have new leads
        //                       to chase here. Tells the operator "we've
        //                       been here before" rather than the misleading
        //                       "no rentals here".
        //   empty    (grey)   - we genuinely found nothing worth qualifying,
        //                       either zero places at all or new places that
        //                       all failed classification.
        const newSurvivors = placesFound - alreadyKnown;
        let finalState;
        if (leadsQualified > 0) {
            finalState = 'complete';
        } else if (placesFound > 0 && newSurvivors === 0) {
            finalState = 'no_new';
        } else {
            finalState = 'empty';
        }
        ...
        await grid.updateCell(cell.id, {
            ...
            alreadyKnown,
            lastScannedAt: Date.now(),
            // Stamp the FULL effective term list from pickTermsForCell (the
            // Maps queries that were valid for this cell), not just the
            // search_log-deduped subset that hit Scrapingdog this run.
            // Coverage's staleness check compares against this stamp to find
            // cells where the ICP has since added a new term.
            searchTerms: allTerms,
            // Clear any prior pause checkpoint on successful completion -
            // we either ran fresh (no checkpoint to begin with) or resumed
            // a checkpointed cell to completion (checkpoint no longer needed).
            pauseCheckpoint: null,
        });
        ...
        // Reset to pending so the next tick retries from scratch. Clear any
        // checkpoint too - on hard error we don't trust the partial state;
        // safer to re-Maps the cell than resume from a possibly-corrupt
        // mid-iteration spot.
        await grid.updateCell(cell.id, {
            state: 'pending',
...
async function runDemoSweep(icp, cell) {
    ...
    // them yet.
    // Same precedence logic as the live sweep - delegated to pickTermsForCell
    // so the two paths can't drift. Outlier cells (skip case) get a short-
    // circuited "empty" demo run so the UI shows the gray-out honoured.
    const pickDemo = pickTermsForCell(icp, cell);
    if (pickDemo.skip) {
        const cellCountryDemo = String(cell.country || '').toUpperCase();
        console.log(`[Demo Sweep] ⊘ ${cityLabel}/${cell.id.slice(0, 8)} - country=${cellCountryDemo} not in ICP countries (${(icp.countries || []).join(',')}) and no city override - skipping`);
        pushEvent({
            type: 'cell_complete',
            icpId: icp.id,
            cellId: cell.id,
            parentCity: cell.parentCity || null,
            message: `Skipped ${cityLabel} - ${cellCountryDemo || 'unknown country'} not ticked on this ICP`,
        });
        await grid.updateCell(cell.id, {
            state: 'empty',
            placesFound: 0,
