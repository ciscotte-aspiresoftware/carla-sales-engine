...
router.post('/search', async (req, res) => {
    ...
    // When the caller passes an explicit `page`, run just that one (old
    // per-page navigation contract). When they don't, paginate from page 0
    // through the Admin-configured maxPagesPerSearch.
    const explicitPage = Number.isInteger(page) ? page : null;
    const maxPages = explicitPage !== null
        ? 1
        : Math.max(1, Math.min(parseInt(getCellGeneration().maxPagesPerSearch, 10) || 1, 6));
    const startPage = explicitPage ?? 0;
    const pageOffsets = Array.from({ length: maxPages }, (_, i) => startPage + i * 20);
    ...
    try {
        // Sequential page loop. Each page = 5 Scrapingdog credits. Stops
        // early when a page returns < 20 results (Scrapingdog has no more
        // for this query at this location).
        const rawResults = [];
