...
export default function CoverageGlobe({
  ...
  const points = useMemo(() => {
    return cells.map((c) => {
      ...
      //   pending  - sky-300  - sky-blue, queued
      //   scanning - red-400  - red, in flight
      //   complete - emerald  - found new qualified leads
      //   no_new   - amber-400 - swept, places existed but all already known
      //                          (yellow = "we've been here, nothing new")
      //   empty    - slate-400 - swept, genuinely no leads at this resolution
      ...
      else if (c.state === 'no_new') color = '#fbbf24'   // amber-400
  ...
  return (
    <div ref={containerRef} className="relative w-full h-full">
      {/* Loading placeholder shown while the WebGL canvas is still cold -
          shaders compiling, textures uploading, camera not yet snapped to
          target. Fades out in parallel with the canvas fade-in (both 300ms)
          so there's no visual swap-flash. Pointer-events:none on the
          placeholder itself, so clicks pass straight through once the real
          globe takes over. */}
      <GlobePlaceholder hidden={cameraSettled} />
