...
interface Props {
  ...
  connected: boolean
  // True between pressing "Resume sweeping" and the first live event. Shows
  // a spinner so the gap (up to one cron tick) doesn't look idle/stuck.
  starting?: boolean
}

export default function NowSweepingPanel({ progress, connected, starting }: Props) {
  if (!progress) {
    return (
        ...
        <div className="flex items-center gap-2 text-xs">
          {starting ? (
            <>
              ...
              <span className="font-medium text-sky-600 dark:text-sky-400">Starting sweep - fetching first cell…</span>
            </>
          ) : connected ? (
            <>
