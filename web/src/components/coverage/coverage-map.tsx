...
function FlyTo({
  ...
  const map = useMap()
  // Dedupe by value, not by reference. The parent's `zoomTarget` recomputes
  // every cells-poll (6s) producing a new `bounds` array even when the
  // numeric values are identical - that was re-firing flyToBounds and
  // yanking the user back to the country bbox while they were trying to
  // zoom in. Tracking the last-applied key prevents redundant flights.
  ...
  useEffect(() => {
    if (bounds) {
      ...
      lastCenterKeyRef.current = null // bounds path supersedes the point-fly path
...
function CellMarkers({
  ...
  const markers = useMemo(() => {
    return cells.map((c) => {
      ...
      else if (c.state === 'no_new')   { color = '#b45309'; fillColor = '#fbbf24' } // amber-700 / amber-400
