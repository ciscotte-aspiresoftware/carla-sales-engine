...
// Sidebar organized into three groups that mirror the lead-funnel
// mental model:
//
//   (top, no label)   Daily-action surfaces - where the user lives most
//                     of the day.
//   PIPELINE          The lead discovery + qualification flow, in order:
//                       Coverage (machine discovery) →
//                       New Leads (one-off sourcing) →
...
export function AppSidebar() {
  ...
  return (
    ...
    >
      ...
      {/* Footer mode pill. Clickable → /admin so the operator can flip
          demo ↔ real without an explicit nav item. Stays out of the main
          nav groups so it doesn't get mixed up with daily-action items. */}
      <NavLink
        to="/admin"
        title={collapsed ? `Mode: ${mode} — open Admin` : 'Open Admin'}
        className={({ isActive }) => cn(
          'border-t border-white/30 dark:border-white/10 px-3 py-3 text-[10px] flex items-center gap-2 transition-colors hover:bg-white/20 dark:hover:bg-white/5',
          isActive && 'bg-white/30 dark:bg-white/[0.05]',
          collapsed && 'justify-center',
        )}
      >
        <span
        ...
        />
        {!collapsed && (
          <span className="flex-1 min-w-0 flex items-center justify-between text-muted-foreground">
            <span className="uppercase tracking-wider">{mode === 'real' ? 'Real mode' : 'Demo mode'}</span>
            <span className="text-[9px] opacity-60">Admin →</span>
          </span>
        )}
      </NavLink>
    </aside>
